...
def generate_dataset(num_vars, num_clauses,instances,DIR_PATH):
    ...
    for retry in range(max_retries):
        ...
        if balance_ratio >= balance_threshold:
            ...
            print(f"Balance threshold met. Exiting loop. Retry: {retry}")
            break
...
def main(num_vars, num_clauses, model_path,instances,output_path,directory_path):
    ...
    equi_loss, precision, recall, f1_score, confusion_matrix = batch_equisatisfiability_check(predicted_cnfs, ground_truth_cnfs, valid_predictions_count,alpha = 0.5)
    print(f"Precision: {round(precision,2)}")
    print(f"Recall: {round(recall,2)}")
    print(f"f1_score: {round(f1_score,2)}")
    metrics = {
        'equi_loss': round(equi_loss, 2),
